<?php
	...
	class AppGiniPlugin {
		...
		private $_which_axp; /* which axp file in `projects` matches the current app */

		/**
		 *  Initiate the class.
		 *  @param (array) $config  Associative array containing the following keys:
		 ...
		 *  
		 *  @return (object)  the plugin object that you can use to invoke the various plugin methods.
		 * 
		 *  @example:
		 *      ```
		...
		public function process_ajax_upload() {
			...
			try {
				...
				if( !preg_match('/^[\w\.-]+\.axp$/i', $_FILES['uploadedFile']['name'] )) {
		...
		/**
		 * Retrieve a list of child tables of a given table
		 *  
		 * @param (string) $tn table name
		 * @return (array) a numeric array of all tables that have a foreign key (lookup) field pointing to the given parent table
		 */		
		 ...
		 *  
		 *  @param (object) $pxml optional project xml object. If not provided, `->project_xml` is used.
		 ...
		 *  
		 *  @param (string) $path optional path to projects folder
		 ...
		 * @return (string) primary key field name for given table or false if not found
		 */
		...
		/**
		 * @param (string) $table_name name of table
		 * @return array  field names of table or `false` on error
		*/
		...
		/**
		 * @param string $tn  name of table
		 * @return object corresponding to given table name if exists, `false` otherwise
		*/
		function table($tn) {
		...
		/**
		 * @param string $tn  name of table
		 * @param string $fn  name of field
		 * @return object field corresponding to given field name if exist, `false` otherwise
		*/
		 ...
		 * @param string $tn  name of table
		 * @param string $fn  name of field
		 * @return int 0-based field index for given table and field names if exist, -1 otherwise
		*/
		...
		/**
		 * @param string $tn  name of table
		 * @return int 0-based index of given table name if exists, -1 otherwise
		*/
		...
		/**
		 * @brief Retrieve the owner of the current process
		 * @return string the owner of the current process, or empty string if not supported (e.g. on Windows)
		 */
		...
		/**
		 * @brief Retrieve the owner user of a given path
		 * @param [in] $path path to check
		 * @return string the owner of the given path, or empty string if not supported (e.g. on Windows)
		 */
		public function path_owner($path) {
			if(!function_exists('posix_getpwuid')) return '';
