$j(function() {
	...
	var saveCommand = function() {
		...
		$j.ajax({
			...
			data: {
				...
				triggerUpdateHooks: $j('#trigger-update-hooks').prop('checked') + 1 - 1, // converts boolean to 0/1
				hash: val('command-id'),
			},
	...
	var initCommandsList = function() {
		...
		AppGiniPlugin.commandsList = AppGiniPlugin.itemsList({
			...
			itemDetails: function(cmd) {
				...
				return {
					'Field': `<code>${cmd.field}</code> ${cmd.triggerUpdateHooks ? '<i class="glyphicon glyphicon-flash" title="Triggers hooks"></i>' : ''} ${(!field ? '<i class="hspacer-lg text-bold text-danger">No longer exists!</i>' : '')}`,
	...
	var buildCommandFormIfNeeded = function() {
		...
		AppGiniPlugin.commandForm
			...
			.add({
				id: 'fixed-value',
				type: 'textarea',
				help: 'Enter the value to set this field to. HTML code allowed, but won\'t work for numeric/date fields. For lookup fields, enter the ID of the lookup value rather than the value itself.',
				label: 'Fixed value',
				groupClasses: 'hidden',
			...
			.add({
				id: 'trigger-update-hooks',
				label: `Trigger <a href="https://bigprof.com/appgini/help/advanced-topics/hooks/table-specific-hooks#tablename_before_update" target="_blank">update hooks</a>
						<span class="text-italic text-muted">(don't check this option if the field you are updating is a hidden field)</span>`,
				type: 'checkbox',
				init: false
			})
			.add({
	...
	var loadCommand = function(cmdId) {
		...
		$j('#trigger-update-hooks').prop('checked', cmd.triggerUpdateHooks == 1);
