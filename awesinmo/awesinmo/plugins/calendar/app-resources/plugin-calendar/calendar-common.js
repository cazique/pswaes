$j(function() {
	...
	AppGini.Calendar = {
		...
		newEventButtons: {
			...
			show: function(selection, singleEvent) {
				$j('.new-event').each(function(newEventIndex) {
					// if singleEvent is set and is not equal to newEventIndex, hide this button and return
					if(singleEvent !== undefined && singleEvent != newEventIndex) {
						$j(this).addClass('hidden').data('event', null);
						return;
					}

					$j(this).removeClass('hidden').data('event', {
						'calendar.startDate': moment(selection.start).format('YYYY-MM-DD'),
						'calendar.startTime': moment(selection.start).format('HH:mm:ss'),
						'calendar.endDate': moment(selection.end).format('YYYY-MM-DD'),
						'calendar.endTime': moment(selection.end).format('HH:mm:ss'),
						'calendar.allDay': selection.allDay,
						'calendar.newEventType': $j(this).data('type')
					});
				});
			},

			hide: function(singleEvent) {
				setTimeout(function() {
					if(singleEvent !== undefined) {
						$j('.new-event').eq(singleEvent).addClass('hidden').data('event', null);
						return;
					}
					...
					$j('<div><i class="glyphicon glyphicon-plus"></i></div>')
						...
						.on('click', function() {
							...
							modal_window({
								...
								close: function() {
									// if `Calendars` is defined and length > 1, reload all calendars
									if(typeof(Calendars) !== 'undefined' && Calendars.length > 1) {
		...
		compactHeight: function(scrollTime) {
			if(undefined === this._fullCal) return;
			
			this._fullCal.setOption('height', undefined);
			this._fullCal.setOption('contentHeight', undefined);

			if(undefined === scrollTime) return;

			this._fullCal.scrollToTime(scrollTime);
		},
		 ...
		 *
		 *	Generate a span element with data-key and data-replace attributes:
		 *	    span = AppGini.Calendar.Translate.html('key', replacements);
		 *	    
		 *	Change language:
		...
		*/
		Translate: {
			...
			word: function(key, replace) {
				...
				if(undefined === parent.language) return false;
			...
			html: function(key, replace) {
				let span = `<span class="language" data-key="${key.safeAttr()}"`;
				if(replace !== undefined) {
					for(var r in replace) {
						if(!replace.hasOwnProperty(r)) continue;
						span += ` data-${r}="${replace[r].safeAttr()}"`;
					}
				}
				span += '></span>';
				return span;
	...
	// Add safeAttr() method to String prototype
	// Example: 'this & that'.safeAttr() returns 'this &amp; that'
	String.prototype.safeAttr = function () {
		return this.replace(/&/g, "&amp;")   // Encode ampersand
