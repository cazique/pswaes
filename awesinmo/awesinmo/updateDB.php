<?php
	...
	if($thisMD5 != $prevMD5) {
		// set up tables
		setupTable('actividades', []);

		setupTable('actividades_encargos', []);

		setupTable('agencias', []);

		setupTable('clientes', []);

		setupTable('detalles_procedencia', []);

		setupTable('direcciones', []);

		setupTable('encargos', []);

		setupTable('estados_contacto', []);

		setupTable('estados_encargo', []);

		setupTable('estados_noticia', []);

		setupTable('inmuebles', []);

		setupTable('modalidades_contacto', []);

		setupTable('motivos_cierre', []);

		setupTable('noticias', []);

		setupTable('prioridades', []);

		setupTable('subtipologias_inmueble', []);

		setupTable('subzonas', []);

		setupTable('tipologias_inmueble', []);

		setupTable('tipos_actividad', []);

		setupTable('tipos_procedencia', []);
