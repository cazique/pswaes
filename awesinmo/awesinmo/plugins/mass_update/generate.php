<?php 
	...
	function command_ajax_file($mu, $path, $tn, $cmd) {
		...
		echo $php_begin . "\n";
		?>
		/* mass_update: Applying command: '<?php echo $cmd->label; ?>' to <?php echo $tn; ?> table */
		$allowed_groups = <?php echo groups_array_php($cmd); ?>;

		$triggerUpdateHooks = <?php echo (($cmd->triggerUpdateHooks ?? false) ? 'true' : 'false'); ?>;
		...
		if(!$triggerUpdateHooks || !function_exists('curl_init')) {
		...
		// perform update request in a manner that triggers hooks
		$requests = [];
		$res = sql("SELECT * FROM `{$tn}` WHERE `{$pk}` IN ({$cs_ids})", $eo);
		while($row = db_fetch_assoc($res)) {
		﹣﹣$data = $row;
		<?php if($cmd->value == 'toggle') { ?>
			﹣﹣$data[$field] = $data[$field] ? '0' : '1';
		<?php } elseif($cmd->value == 'allowUserToSpecify') { ?>
			﹣﹣$data[$field] = ($new_value == 'MASS_UPDATE_TOGGLE_CHECKBOX' ? ($data[$field] ? '0' : '1') : $new_value);
		<?php } else { ?>
			﹣﹣$data[$field] = $new_value;
		<?php } ?>
		﹣﹣$data['SelectedID'] = $data[$pk];

		﹣﹣$ch = prepareCurlUpdateRequest($tn, $data);
		...
		function prepareCurlUpdateRequest($tn, $data) {
		﹣﹣$ch = curl_init();
		﹣﹣$payload = $data;
		﹣﹣$payload['update_x'] = 1;
		...
		﹣﹣$token = jwt_token();
		﹣﹣$options = [
		﹣﹣﹣﹣CURLOPT_URL => application_url("{$tn}_view.php"),
		﹣﹣﹣﹣CURLOPT_POST => true,
		...
		for($i = 0; $i < count($commands); $i++) {
			...
			function massUpdateCommand_<?php echo $cmd->hash; ?>(tn, ids) {
				<?php if($cmd->value == 'allowUserToSpecify') { ?>
					...
					﹣﹣﹣﹣﹣﹣﹣﹣﹣﹣success: function() { $j('form').get(0).reset(); $j('form').eq(0).trigger('submit'); },
					﹣﹣﹣﹣﹣﹣﹣﹣﹣﹣error: function() {
